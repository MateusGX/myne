...
This guide explains the multi-language support system in Myne.
...
More languages are welcome — see [translators.md](./translators.md) for how to contribute one.
...
The selected language is stored as the `language` field in `/.myne/settings.json`, alongside all other
device settings (see [src/MyneSettings.cpp](../src/MyneSettings.cpp)).

A legacy standalone `/.myne/language.bin` (1-byte version + 1-byte language index, using the frozen V1
enum order from `I18nKeys.h`) is migrated into `settings.json` automatically if found, then renamed to
`language.bin.bak`.
