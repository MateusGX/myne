...
enum class ReadingStatus : uint8_t { WantToRead = 0, Reading = 1, Paused = 2, Finished = 3, Dropped = 4 };
...
struct Reading {
  ...
  int lastPosition() const { return sessions.empty() ? 0 : sessions.back().position; }
...
class ReadingLog {
 public:
  ...
  bool saveForBook(const std::string& bookId, const std::vector<Reading>& readings) const;

  // Load the compact binary summary (lazy-generates from JSON on first call).
  // Returns false if no readings exist for this book.
  bool loadSummaryForBook(const std::string& bookId, ReadingSummary& out) const;
  ...
  void rebuildStats(const char* booksDirPath, void (*onProgress)(void* ctx, int done, int total) = nullptr,
 ...
 private:
  ...
  void saveSummaryBinary(const std::string& bookId, const std::vector<Reading>& readings) const;
};
