...
MYNE_VERSION for the default (dev) environment.

Results in a version string like:  1.1.0-dev-feat-kosync-xpath-05c6cf8
Release environments are unaffected; they set MYNE_VERSION in the ini.
"""
...
def get_base_version(project_dir):
    ...
    if not config.has_option('myne', 'version'):
        warn('No [myne] version in platformio.ini; base version will be "0.0.0"')
        return '0.0.0'
    return config.get('myne', 'version')
...
def inject_version(env):
    ...
    env.Append(CPPDEFINES=[('MYNE_VERSION', f'\\"{version_string}\\"')])
    print(f'Myne build version: {version_string}')
