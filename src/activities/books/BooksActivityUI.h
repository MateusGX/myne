...
inline void hero(const GfxRenderer& r, Rect rect, const char* eyebrow, const char* title, const char* detail = nullptr,
                 int rightReserve = 0) {
  panel(r, rect, true);
  text(r, SMALL_FONT_ID, rect.x + INNER, rect.y + 14, eyebrow, rect.width - INNER * 2 - rightReserve,
       EpdFontFamily::BOLD);
  text(r, UI_10_FONT_ID, rect.x + INNER, rect.y + 40, title, rect.width - INNER * 2 - rightReserve,
       EpdFontFamily::BOLD);
  if (detail && detail[0] != '\0') {
    text(r, SMALL_FONT_ID, rect.x + INNER, rect.y + 67, detail, rect.width - INNER * 2 - rightReserve);
  }
}
