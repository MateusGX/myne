...
const char* dashIfEmpty(const std::string& s) {
...
void drawLabelValue(const GfxRenderer& renderer, Rect rect, const char* label, const char* value,
  ...
  if (maxLines <= 1) {
    const auto safe = renderer.truncatedText(valueFont, value, rect.width,
                                             valueFont == UI_10_FONT_ID ? EpdFontFamily::BOLD
...
void drawMetaCard(const GfxRenderer& renderer, Rect rect, const char* label, const char* value) {
  ...
  drawLabelValue(renderer, Rect{rect.x + kInner, rect.y + 14, rect.width - kInner * 2, rect.height - 24},
                 label, value);
}
...
void PhysicalBookDetailActivity::onExit() {
  Activity::onExit();
}
...
void PhysicalBookDetailActivity::loop() {
  ...
  if (mappedInput.wasReleasedGroup(MappedInputManager::ButtonGroup::BottomRight)) {
    startActivityForResult(
        std::make_unique<BookReadingStatsActivity>(renderer, mappedInput, book),
...
void PhysicalBookDetailActivity::render(RenderLock&&) {
  ...
  // Hero
  {
    const auto author = renderer.truncatedText(SMALL_FONT_ID, dashIfEmpty(book.author),
                                               CW - BooksActivityUI::INNER * 2);
    BooksActivityUI::hero(renderer,
                          Rect{kPad, y, CW, BooksActivityUI::HERO_H},
                          tr(STR_BOOK_DETAIL), book.title.c_str(), author.c_str());
  ...
  // Metadata grid
  {
    ...
    drawMetaCard(renderer, Rect{kPad + cardW + gap, y, cardW, cardH}, tr(STR_BOOK_LOCATION), dashIfEmpty(book.location));
    y += cardH + 12;
  }
  ...
  // Notes
  {
    ...
    drawLabelValue(renderer, Rect{kPad + kInner, y + 14, CW - kInner * 2, notesH - 24},
                   tr(STR_BOOK_NOTES), notes, SMALL_FONT_ID, 3);
    y += notesH + 12;
  }
  ...
  // Reading summary / action card
  {
    ...
    drawLabelValue(renderer, Rect{tx + colW + kInner, rowY, colW, 52},
                   tr(STR_LOG_POSITION), posBuf[0] ? posBuf : "-");

  }

  const auto btnLabels = mappedInput.mapLabels(tr(STR_BACK), tr(STR_READINGS),
