...
inline void metric(const GfxRenderer& r, Rect rect, const char* label, const char* value,
                   bool selected = false) {
  panel(r, rect, selected);
  text(r, SMALL_FONT_ID, rect.x + INNER, rect.y + 14, label, rect.width - INNER * 2,
       EpdFontFamily::BOLD);
  text(r, UI_10_FONT_ID, rect.x + INNER, rect.y + 42, value, rect.width - INNER * 2,
       EpdFontFamily::BOLD);
}
...
inline void choice(const GfxRenderer& r, Rect rect, const char* title, const char* detail,
  ...
  text(r, UI_10_FONT_ID, x, rect.y + 18, title, rect.width - INNER * 2 - (selected ? 8 : 0),
       EpdFontFamily::BOLD);
...
inline void signalBars(const GfxRenderer& r, int x, int y, int rssi) {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    const int by = y + 20 - h;
    if (i < bars) r.fillRect(bx, by, bw, h, true);
    else r.drawRect(bx, by, bw, h, true);
  }
}
