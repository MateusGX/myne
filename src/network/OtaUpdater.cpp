...
namespace {
constexpr char latestReleaseUrl[] = "https://api.github.com/repos/MateusGX/myne/releases/latest";

esp_err_t http_client_set_header_cb(esp_http_client_handle_t http_client) {
  return esp_http_client_set_header(http_client, "User-Agent", "Myne-ESP32-" MYNE_VERSION);
}
...
OtaUpdater::OtaUpdaterError OtaUpdater::checkForUpdate() {
  ...
  LOG_DBG("OTA", "Checking for update (current: %s)", MYNE_VERSION);
  ...
  esp_err = esp_http_client_set_header(client_handle, "User-Agent", "Myne-ESP32-" MYNE_VERSION);
  if (esp_err != ESP_OK) {
...
bool OtaUpdater::isUpdateNewer() const {
  if (!updateAvailable || latestVersion.empty() || latestVersion == MYNE_VERSION) {
    return false;
  }
  ...
  const auto currentVersion = MYNE_VERSION;
